...
3. Need to cover Azure AI Search and need to revisit https://learn.microsoft.com/en-us/training/modules/search-azure-cosmos-db-sql-api-data-azure-cognitive-search/. Basically it's the same on how we create OpenAI, just that this is by creating an indexer it auto pulls from CosmosDB (without vector creation) and from there we can determine which fields are sortable, filterable, etc.
3. Embedding refresh
